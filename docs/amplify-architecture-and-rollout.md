...
Hand written HTML. No framework, no bundler, no package manager, no build
step, no server code, no database, and no secrets. Rebuilt on 2026-09-08 from
a single anchored page into ten pages sharing one stylesheet and one script.

```
/                     index.html                  home
/paid-media/          paid-media/index.html       service page
/ai-systems/          ai-systems/index.html       service page
/workshops/           workshops/index.html
/case-studies/        case-studies/index.html     index of the three below
/pricing/             pricing/index.html
/learn/               learn/index.html
/about/               about/index.html
/contact/             contact/index.html          audit capture form
/ai-instructions/     ai-instructions/index.html  written for answer engines
/cs-dre.html                                      case study
/cs-nonprofit.html                                case study
/cs-advocacy.html                                 case study, noindex, unlinked
/404.html                                         not found page
/robots.txt  /sitemap.xml                         search directives
/assets/                                          site.css, site.js, images
```

**The design system is shared across the ten pages, not across all fourteen.**
Those ten load `assets/site.css` and `assets/site.js`, so the header, footer,
navigation, drawer and form behave identically on each. The three case studies
and the 404 predate the rebuild and still carry their own inline `<style>`
blocks. Any change to a color, a type scale or the header has to be made in
both places until they are reconciled, which is the same class of drift that
produced two visual identities before.

The audit capture form on `/contact/` is the site's only lead capture path. It
carries no `action` or `method`; the script posts to the endpoint named in
`data-endpoint`, and the submit button ships disabled and is enabled only once
a real form provider access key is present. Until that key is configured the
...
2. Run Lighthouse on mobile and desktop and compare against the scores the
   current production deployment gets. A change may not regress them.
